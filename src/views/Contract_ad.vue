<template>
    <template v-if="isLoading">
        ...
        <ul class="breadcrumb breadcrumb-style ">
            ...
            <li class="breadcrumb-item">
                <router-link :to="`/contract/${templateData.temId}/list`">
                    <vue-feather type="link"></vue-feather>
                    {{ templateData.temTitle }} 列表
                </router-link>
            </li>
                    ...
                    <div v-if="area.areaType === '1'" class="card contract-title">
                        <div class="author-box-name d-flex justify-content-between"
                            ...
                            <div class="contract-serial">
                                <!-- 這裡放文件序號 -->
                                <div style="font-weight: 400;">文件序號：<span
                                        class="date">{{ contractData.conSerial }}{{ contractData.conVer }}</span>
                                </div>
                                <!-- 這裡放創文日期 -->
                        ...
                        <div class="card-body myNotification d-flex">
                            <div class="myFont16Title" style="margin: 0 10px;">申請單位： <span class="date myFont16">{{ this.per.perBu1}}  {{ this.per.perBu2 }}</span>
                            </div>
                            <div class="myFont16Title" style="margin: 0 10px;">申請人： <span class="date myFont16">{{ this.per.perName }}</span>
                            </div>
                            <div class="myFont16Title" style="margin: 0 10px;">聯絡電話： <span
                                    class="date myFont16">{{ this.per.perPhone1 }} {{ this.per.perPhone2 }} {{ this.per.perPhone3 }}</span>
                            </div>
                        </div>
                        <div class="card-body myNotification d-flex">
                            <div class="myFont16Title" style="margin: 0 10px;">申請類型：
                                <span v-if="'0' === conType" class="date myFont16">新增</span>
                                <span v-if="'1' === conType" class="date myFont16">變更</span>
                                <span v-if="'2' === conType" class="date myFont16">終止</span>
                            </div>
                        </div>
                    </div>
                                ...
                                <template v-for="(col, childIndex) in area.colItem" class="row">
                                    <div :class="['col-' + col.width + ' sub-item']">
                                        <template v-if="col.type === 'subject'">
                                            ...
                                            <div class="row">
                                                <div class="d-flex justify-content-between"
                                                    ...
                                                    <input type="text" v-model="contractData.conTitle" class="form-control"
                                            ...
                                            <div class="row">
                                                <div class="d-flex justify-content-between"
                                                    ...
                                                    <select class="form-control" v-model="contractData.frmId">
                                                        <option value="0">請選擇框架</option>
                                                        <option v-for="frm in frameData" :value="frm.frmId">{{
                                                            frm.frmTitle }}
                                                        </option>
                                                    </select>
                                                </div>
                                            ...
                                            <div class="row">
                                                <div class="d-flex justify-content-between"
                                                    ...
                                                    <input disabled type="text"
                                                           :value="this.$root.getCompanyTitle('', contractData.comCode)"
                                        ...
                                        <template v-if="col.type.startsWith('list')">
                                            <label v-if="col.name !== '' || col.tip !== ''" class="myFont16 p-t-10">{{
                                                col.name }}
                                                <vue-feather v-if="col.tip !== ''"
                                                             v-tooltip="{ content: col.tip, placement: 'right' }"
                                                             type="help-circle" size="20" stroke="blue"></vue-feather>
                                        ...
                                        <template v-if="col.type.startsWith('word')">
                                            ...
                                            <template v-for="cat in categoryData">
                                                <div v-if="parseInt(cat.catId) === parseInt(col.id)" class="row">
                                        ...
                                        <template v-if="col.type === 'sign'">
                                            ...
                                            <div class=" mt-2 d-flex justify-content">
                                                <button type="button" @click="addMember('U', per.perBu1Code)"
                                                        class="btn btn-icon icon-left btn-primary myFont16"
                                                        style="border-radius: 6px;">新增一筆
                                                </button>
                                            </div>
                                        ...
                                        <template v-if="col.type === 'file_area'">
                                            <label v-if="col.name !== '' || col.tip !== ''" class="myFont16 mb-1">{{
                                                col.name }}
                                                <vue-feather v-if="col.tip !== ''"
                                                             v-tooltip="{ content: col.tip, placement: 'right' }"
                                                             type="help-circle" size="20" stroke="blue"></vue-feather>
                                            ...
                                            <div class="d-flex justify-content-between align-center"
                                                ...
                                                <label class="myFont16 col-4 p-t-10">{{ filMeetingFiles.length +
                                                    filPlanFiles.length + filOtherFiles.length }}則附加檔案</label>
                                                <!-- 這裡放附檔 -->
                                                <div>
                                                    <FileUpload
                                        ...
                                        <template v-if="col.type === 'radio'">
                                            <div class="d-flex justify-content-between align-center"
                                                ...
                                                <div v-for="option in col.option.split('|')" class="form-check p-t-10">
                                                    ...
                                                    <label class="form-check-label"
                                                           :for="'rdo_'+parentIndex+'_'+childIndex">{{ option }}</label>
                                                </div>
                                            </div>
                                        </template>
                                        <template v-if="col.type === 'check'">
                                            <div class="d-flex justify-content-between align-center"
                                                ...
                                                <div v-for="option in col.option.split('|')" class="form-check p-t-10">
                                                    ...
                                                    <label class="form-check-label"
                                                           :for="'cbx_'+parentIndex+'_'+childIndex">{{ option }}</label>
                                                </div>
                                            </div>
                                        </template>
                                        ...
                                        <template v-if="col.type === 'box'">
                                            <div class="row" style="margin-bottom: 20px;">
                                                ...
                                                <textarea class="my-form-control" v-model="col.value" spellcheck="false"
                                                          placeholder="請輸入標題"  style="margin-left: 10px;"></textarea>
                                            </div>
                                        </template>
                                    </div>
                                </template>
        ...
        <div class="col-6" style="padding-bottom: 20px;">
            <button type="button" @click="updateContract"
                    :disabled="contractData.conTitle === '' || conCompany.length === 0 || conWork.length === 0"
...
<script>
    ...
    export default {
        name: "Contract_ad",
        mixins: [memberMixin, itemMixin, fileMixin, controlBoxMixin, contractActionMixin],
        data() {
            return {
                isLoading: false,
        ...
        methods: {
            ...
            fetchFirst() {
                ...
                Promise.all(apiRequests)
                    .then(([contractResponse, templateResponse, workResponse, companyResponse, frameResponse, categoryResponse, sourceResponse, distributionResponse, mannerResponse, contactResponse]) => {
                        //contractResponse
                        ...
                        if (this.contractData.conValue && '' !== this.contractData.conValue && 'NULL' !== this.contractData.conValue) {
                            this.conValue = this.contractData?.conValue ? JSON.parse(this.contractData.conValue) : (this.templateData?.temStyle ? JSON.parse(this.templateData.temStyle) : null);
                        }
                        else {
            ...
            async updateContract() {
                    ...
                    conValue.forEach(area => {
                        if (area.areaType === '2') {
                            this.expData.forEach(exp => {
                                exp.comValue = parseInt(exp.comValue);
                                exp.comPercent = parseFloat(exp.comPercent);
                            });
                            area.areaValue = this.expData;
                        }
                        area.colItem.forEach(col => {
                            ...
                            if (col.type === 'check') {
                                const selectedOptions = col.checkedOptions.filter(option => option !== '');
                                col.value = selectedOptions.join('|');
                                delete col.checkedOptions;
                            }
                        });
                    });
                }
                ...
                await this.saveContract(payload, this.contractData.temId, this.contractData.conId, null);
